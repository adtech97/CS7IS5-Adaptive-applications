...
def side_bar_diet_search():
    ...
    max_time = st.sidebar.selectbox("Max Time", [i for i in range(1, 100)])
    submit = st.sidebar.button("Submit")
    theme_bad = {'bgcolor': '#FFF0F0','title_color': 'red','content_color': 'red','icon_color': 'red', 'icon': 'fa fa-times-circle'}
    theme_neutral = {'bgcolor': '#f9f9f9','title_color': 'orange','content_color': 'orange','icon_color': 'orange', 'icon': 'fa fa-question-circle', 'font_size': '10px'}
    theme_good = {'bgcolor': '#EFF8F7','title_color': 'green','content_color': 'green','icon_color': 'green', 'icon': 'fa fa-check-circle', 'font_size': '20px'}
    ...
    with cols[0]:
        if bmi < 18.5:
            hc.info_card(title = "BMI", content = f"{bmi:.2f} bmi is low than expected", bar_value=int(bmi), theme_override=theme_bad)
    ...
    with cols[1]:
        hc.info_card(title = "Protein Intake", content = f"{protein_intake:.2f} grams", bar_value=100, sentiment="neutral", theme_override=theme_neutral)
    with cols[2]:
        hc.info_card(title = "Calories", content = f"{calories_required:.2f} kcal", bar_value=100, sentiment="nuetral", theme_override=theme_neutral)
    if submit:
        st.divider()
        st.sidebar.subheader("Your Diet Search Results")
        ...
        st.write("According to your preferences, here are some diet recommendations divide them asper your meal plan")
        #st.write(df)

        view_df = df[['Name', 'RecipeIngredientParts', 'RecipeInstructions' ,'TotalTime', 'Description', 'Calories', 'ProteinContent']]
        view_df['TotalTime'] = view_df['TotalTime'].apply(lambda x: x.split("T")[1].replace("H", ":").replace("M", ""))
        view_df['RecipeInstructions'] = view_df['RecipeInstructions'].apply(lambda x: eval(x.replace("c(", "[").replace(")", "]")))
        view_df['RecipeIngredientParts'] = view_df['RecipeIngredientParts'].apply(lambda x: eval(x.replace("c(", "[").replace(")", "]")))
        st.write(view_df)
